<template>
  <div class="bg-white border rounded shadow-xl p-4">
    <div class="flex items-center justify-end mb-2">
      <input v-model="search" type="text" name="search" class="bg-slate-100 text-xs border border-slate-300 rounded shadow w-full sm:w-1/3 placeholder:capitalize" :placeholder="__('search something')">
    </div>

    <div class="overflow-auto border border-slate-300 rounded">
      <table class="border-collapse w-full">
        ...
        <tbody class="bg-slate-50">
          <tr v-for="(role, i) in roles.filter(role => search.trim() ? (role.name.includes(search.trim()) || role.permissions.find(permission => permission.name.includes(search.trim()))) : true)" :key="i" class="border-slate-200 hover:border-slate-300 hover:bg-slate-200">
            <td class="border border-slate-300 px-3 py-1 text-center">{{ i + 1 }}</td>
            <td class="border border-slate-300 px-3 py-1 uppercase">{{ __(role.name) }}</td>
            <td class="border border-slate-300 px-3 py-1">
              <template v-if="role.permissions.length">
                <button v-for="(permission, j) in role.permissions" :key="j" class="px-2 py-1 text-xs text-slate-200 bg-slate-600 border border-slate-700 rounded m-1 uppercase cursor-default">
        ...
        <tfoot class="bg-slate-200">
          <tr>
            <th class="border border-slate-300 px-3 py-1 uppercase">{{ __('no') }}</th>
            <th class="border border-slate-300 px-3 py-1 uppercase">{{ __('name') }}</th>
            <th class="border border-slate-300 px-3 py-1 uppercase">{{ __('permissions') }}</th>
            <th class="border border-slate-300 px-3 py-1 uppercase">{{ __('action') }}</th>
          </tr>
        </tfoot>
      </table>
    </div>
